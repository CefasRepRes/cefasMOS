...
#' Fetch CTD data from CTD postgres database
#'
#' @param survey a survey code, use '%' as a wildcard e.g. "%CEND%18"
...
#' @param parameters a vector of parameter codes default is c("TEMP", "SAL", "O2CONC", "FTU", "FLUORS", "ALT", "PAR"), use parameters = "ALL" to select all.
#' @param binned if TRUE (default) return 1 dbar binned data, otherwise returns raw frequency data
#'
#' @return data.table containing binned CTD data
#' @export
#' @examples
#'\dontrun{
...
ctd.fetch <- function(survey = NA, bbox = NA, after = NA, before = NA, parameters = c("TEMP", "SAL", "O2CONC", "FTU", "FLUORS", "ALT", "PAR"), binned = T){
  ...
  if(!is.na(bbox[1])){
    where = append(where, paste("latitude >", bbox["ymin"], "AND",
           "latitude <", bbox["ymax"], "AND",
           "longitude >", bbox["xmin"], "AND",
           "longitude <", bbox["xmax"]))
  }
  if(!is.na(after)){
    where = append(where, paste0("start > '", after, "'"))
  }
  if(!is.na(before)){
    where = append(where, paste0("start < '", before, "'"))
  }
